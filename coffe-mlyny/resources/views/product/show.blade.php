        ...
        <div class="row g-5">
            ...
            <div class="col-lg-6">
                ...
                <div class="mt-4 d-flex justify-content-between align-items-center gap-3" role="group">
                    ...
                    <a href="{{ route('products.show', ['slug' => $baseSlug . '-25000']) }}"
                        class="btn btn-product btn-outline-secondary {{ Str::endsWith($product->slug, '25000') ? 'active' : '' }}">250
                        g</a>
                    <a href="{{ route('products.show', ['slug' => $baseSlug . '-50000']) }}"
                        class="btn btn-product btn-outline-secondary {{ Str::endsWith($product->slug, '50000') ? 'active' : '' }}">500
                        g</a>
                </div>

                <form autocomplete="off" method="POST" action="{{ route('cart.add', ['id' => $product->id]) }}" class="mt-4 d-flex justify-content-between align-items-center gap-3 w-100">
                    @csrf
